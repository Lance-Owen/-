...
food_point=food()
for i in a:
    snake = pygame.draw.rect(screen, RED, (i[0], i[1], 10, 10))
    ...
    while not do_not:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:  # 点击了退出
                ...
                if event.key == pygame.K_SPACE:
                    do_not = True
        f1.render_to(screen,[200,150],"游戏暂停",fgcolor=BLACK,bgcolor=None,size=50)
        pygame.display.update()  # 对显示窗口进行更新，默认窗口全部重绘
        fclock.tick(fps)  # 窗口刷新速度，每秒300次
